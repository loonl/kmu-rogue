...
{
    protected float rushCoolTime = 4f; // 돌진 쿨타임
    protected float lastRushTime = -3f; // 마지막 돌진 시점

    bool rushReady = true; // 돌진 준비중 여부
    ...
    {
        if (Time.time < lastRushTime + timeForRushReady && rushReady == true)
        {
            rigidbody2d.velocity = Vector2.zero;
        }
